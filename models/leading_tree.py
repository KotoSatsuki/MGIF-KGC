...
class LeadingTree:
    ...
    def GetSubtreeAverageDensity(self, gamma_D, lt_num, Q, Pa):
            """
            计算每颗子树的平均密度
            :param gamma_D: 按照概率排序后的节点索引
            :param lt_num: 子树的数量
            :param Q: 排序后的样本索引
            :param Pa: 每个节点的父节点索引
            :return: 返回每颗子树的平均密度
            """
            # 初始化子树的平均密度列表
            subtree_avg_density = np.zeros(lt_num)

            # 对每个子树计算其平均密度
            for i in range(lt_num):
                # 获取子树AL[i]中的节点索引
                subtree_nodes = self.AL[i]

                # 计算该子树的平均密度
                subtree_avg_density[i] = np.mean(self.density[subtree_nodes])

            # 选取平均密度最大的子树
            max_density_tree = np.argmax(subtree_avg_density)

            return max_density_tree, subtree_avg_density